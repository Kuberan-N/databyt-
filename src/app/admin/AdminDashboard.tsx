...
const PLAN_STYLE: Record<string, { color: string; bg: string; border: string }> = {
  ...
  growth:  { color: "text-[#4B5FA8]",   bg: "bg-[#F0FDFA]",         border: "border-[#3730A3]/20" },
  scale:   { color: "text-[#0D9488]",   bg: "bg-[#FFF1F2]",         border: "border-[#0D9488]/20" },
};
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      {!orgsLoading && totalOverdue > 0 && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          className="flex items-center justify-between px-5 py-3.5 rounded-2xl bg-[#F0FDFA] border border-[#EF4444]/20">
          <div className="flex items-center gap-3">
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {orgsLoading ? (
                ...
                return (
                  <tr key={org.id}
                    className={`border-t border-[#F1F5F9] hover:bg-[#F8F9FC] transition-colors cursor-pointer ${isSelected ? "bg-[#F0FDFA]" : ""}`}
                    onClick={() => setSelectedOrg(isSelected ? org : org)}>
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded-lg bg-[#F0FDFA] border border-[#3730A3]/20 flex items-center justify-center shrink-0">
                    ...
                    <td className="px-4 py-3.5">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium border capitalize ${
                        ...
                        org.status === "onboarding" ? "bg-[#F0FDFA] text-[#3730A3] border-[#3730A3]/20" :
