...
export default function CollectionsPage() {
  ...
  const statusStyle: Record<string, string> = {
    ...
    bounced: "bg-[#ECFDF5] text-[#DC2626]",
    failed:  "bg-[#ECFDF5] text-[#DC2626]",
  };
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        {[
          { label: "Emails Sent This Month", value: loading ? "—" : (stats?.emailsSentThisMonth.toString() ?? "0"), icon: Mail, color: "#059669" },
          ...
          <div>
            {stats.recentComms.map((c, i) => (
              <div key={i} className="flex items-center gap-4 px-5 py-3.5 border-t border-[#F1F5F9] hover:bg-[#FAFAFA] transition-colors">
                <div className="w-7 h-7 rounded-lg bg-[#ECFDF5] border border-[#059669]/20 flex items-center justify-center shrink-0">
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={s.step} className="flex gap-3">
              <div className="w-6 h-6 rounded-full bg-[#ECFDF5] border border-[#059669]/20 flex items-center justify-center shrink-0 mt-0.5">
                <span className="text-[10px] font-bold text-[#059669]">{s.step}</span>
              </div>
              <div>
