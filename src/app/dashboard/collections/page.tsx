  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        {[
          { label: "Emails Sent This Month", value: loading ? "—" : (stats?.emailsSentThisMonth.toString() ?? "0"), icon: Mail, color: "#E8242A" },
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
        ...
        <div className="px-5 py-3.5 border-b border-[#E2E8F0] flex items-center gap-2">
          <Send className="w-4 h-4 text-[#E8242A]" />
          ...
          <div>
            {stats.recentComms.map((c, i) => (
              <div key={i} className="flex items-center gap-4 px-5 py-3.5 border-t border-[#F1F5F9] hover:bg-[#FAFAFA] transition-colors">
                <div className="w-7 h-7 rounded-lg bg-[#FEF2F2] border border-[#E8242A]/20 flex items-center justify-center shrink-0">
                  <Mail className="w-3.5 h-3.5 text-[#E8242A]" />
                </div>
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={s.step} className="flex gap-3">
              <div className="w-6 h-6 rounded-full bg-[#FEF2F2] border border-[#E8242A]/20 flex items-center justify-center shrink-0 mt-0.5">
                <span className="text-[10px] font-bold text-[#E8242A]">{s.step}</span>
              </div>
              <div>
