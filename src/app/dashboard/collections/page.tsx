...
export default function CollectionsPage() {
  ...
  const statusStyle: Record<string, string> = {
    ...
    clicked: "bg-[#F0FDFA] text-[#0D9488]",
    bounced: "bg-[#F0FDFA] text-[#DC2626]",
    failed:  "bg-[#F0FDFA] text-[#DC2626]",
  };
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        {[
          { label: "Emails Sent This Month", value: loading ? "—" : (stats?.emailsSentThisMonth.toString() ?? "0"), icon: Mail, color: "#0D9488" },
          ...
          <div>
            {stats.recentComms.map((c, i) => (
              <div key={i} className="flex items-center gap-4 px-5 py-3.5 border-t border-[#F1F5F9] hover:bg-[#FAFAFA] transition-colors">
                <div className="w-7 h-7 rounded-lg bg-[#F0FDFA] border border-[#0D9488]/20 flex items-center justify-center shrink-0">
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={s.step} className="flex gap-3">
              <div className="w-6 h-6 rounded-full bg-[#F0FDFA] border border-[#0D9488]/20 flex items-center justify-center shrink-0 mt-0.5">
                <span className="text-[10px] font-bold text-[#0D9488]">{s.step}</span>
              </div>
              <div>
