    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-2xl p-6">
        <div className="flex items-start gap-4 mb-6">
          <div className="w-12 h-12 rounded-2xl bg-[#0D9488] flex items-center justify-center shrink-0">
        ...
        <div className="grid sm:grid-cols-2 gap-3 mb-6">
          {reportSections.map((s, i) => (
            <div key={i} className="flex items-start gap-3 p-3.5 rounded-xl bg-[#F8F9FC] border border-[#E2E8F0]">
              <div className="w-7 h-7 rounded-lg bg-[#F0FDFA] border border-[#0D9488]/20 flex items-center justify-center shrink-0 mt-0.5">
                <s.icon className="w-3.5 h-3.5 text-[#0D9488]" />
              </div>
              <div>
        ...
        <button
          ...
          className="flex items-center gap-2 px-6 py-3 bg-[#0D9488] text-white rounded-xl font-semibold text-sm hover:bg-[#0F766E] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {generating ? (
